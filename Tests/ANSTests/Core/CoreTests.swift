...
func nameParsesWithUnqualifiedTypeName() throws {
    let name = try Name(rawValue: "ans://v1.2.3.agent.example.com")

    #expect(name.version == (try Version("1.2.3")))
    #expect(name.host == (try Host(rawValue: "agent.example.com")))
    #expect(name.rawValue == "ans://v1.2.3.agent.example.com")
}

@Test(.timeLimit(.minutes(1)))
func moduleSelectorDisambiguatesNameCollision() throws {
    struct Name: Equatable {
        let value: String
    }

    let local = Name(value: "local")
    let ans = try ANS::Name(rawValue: "ans://v1.0.0.agent.example.com")

    #expect(local.value == "local")
    #expect(ans.host == (try Host(rawValue: "agent.example.com")))
}

@Test(.timeLimit(.minutes(1)))
func versionRejectsPrereleaseAndBuildMetadata() {
    #expect(throws: (any Error).self) {
        try Version("1.0.0-beta")
    }
    #expect(throws: (any Error).self) {
        try Version("1.0.0+build")
    }
}
...
func hostRejectsIPAndSingleLabel() {
    #expect(throws: (any Error).self) {
        try Host(rawValue: "127.0.0.1")
    }
    #expect(throws: (any Error).self) {
        try Host(rawValue: "localhost")
    }
}

@Test(.timeLimit(.minutes(1)))
func wireAliasesCanonicalize() {
    #expect(Endpoint.ProtocolKind("HTTP_API").rawValue == "HTTP-API")
    #expect(Endpoint.TransportKind("STREAMABLE_HTTP").rawValue == "STREAMABLE-HTTP")
    #expect(Endpoint.TransportKind("JSON_RPC").rawValue == "JSON-RPC")
}
...
func fingerprintSHA256() {
    ...
    let fingerprint = Fingerprint.sha256(der: data)
...
func agentDecodesANSWireKeys() throws {
    ...
    #expect(agent.id == Agent.ID("agent-1"))
    #expect(agent.entryID == Entry.ID("entry-1"))
    #expect(agent.name?.rawValue == "ans://v1.0.0.agent.example.com")
    #expect(agent.host == (try Host(rawValue: "agent.example.com")))
